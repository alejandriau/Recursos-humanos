...
@section('title', 'Crear AFP')

@section('contenido')
<div class="card">
    <div class="card-header">
        <h5 class="mb-0">Crear Nuevo AFP</h5>
    </div>
    <div class="card-body">
        <form action="{{ route('afps.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('cua') is-invalid @enderror"
                               id="cua" name="cua" value="{{ old('cua') }}" maxlength="45" required
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('observacion') is-invalid @enderror"
                          id="observacion" name="observacion" rows="3" maxlength="500">{{ old('observacion') }}</textarea>
                @error('observacion')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
